...
for (n = 0; n < v.length; n++) {
	im[n] = v[n].hasAttribute('data-img')
		? v[n].getAttribute('data-img')
		: 'http://2.bp.blogspot.com/-krXgAqXpPPw/VgkPK7s9CMI/AAAAAAAAB1Y/IVvHmrRYMDI/s1600/dynamic_iframe.png';
}
...
if (v.length > 0) {
	...
	s.type = 'text/css';
	s.innerHTML =
		'.codegena_iframe{overflow:hidden;position:relative;}' +
		...
		'.codegena_iframe .thumb:hover{transform: scale(1.2);transition: all 1s cubic-bezier(0.71, 0.15, 0.37, 0.82);}' +
...
for (n = 0; n < v.length; n++) {
	...
	y.onclick = function() {
		...
		if (this.hasAttribute('data-class') == true && this.getAttribute('data-class') != '') {
			t.classList.add(this.getAttribute('data-class'));
		}
